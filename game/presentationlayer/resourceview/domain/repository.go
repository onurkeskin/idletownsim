...
type IResourceViewRepositoryFactory interface {
	New(db domain.IDatabase) IResourceViewRepository
}

type IResourceViewRepository interface {
	CreateResourceView(buil IResourceView) error
	GetResourceViews() IResourcesView
	GetResourceViewById(id string) (IResourceView, error)
	/*
